...
import { FACTORY_ADDRESS, ZERO_BI, ONE_BI, ZERO_BD, ADDRESS_ZERO, poolsList, factoryContract} from './../utils/constants'
...
export function handlePoolCreated(event: PoolEvent): void {
  ...
  if(poolsList.includes(event.params.pool.toHexString())){
  ...
  pool.feeZtO = BigInt.fromI32(100)
  pool.feeOtZ = BigInt.fromI32(100)
  pool.createdAtTimestamp = event.block.timestamp
  pool.createdAtBlockNumber = event.block.number
  pool.liquidityProviderCount = ZERO_BI
  pool.tick = BigInt.fromI32(887617) // maxTick+1, for null cases
